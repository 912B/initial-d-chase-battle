...
function CB_Battle.OnStart(gridIndex)
    ...
    if myGrid ~= -1 then
        ...
        if ac.ext and ac.ext.physicsSetCarPosition then
             ...
             local myIndex = ac.getSim().focusedCar
             ac.ext.physicsSetCarPosition(myIndex, spawn.position, spawn.look, spawn.up)
             ac.ext.physicsSetVelocity(myIndex, vec3(0,0,0), vec3(0,0,0))
        end
    end
